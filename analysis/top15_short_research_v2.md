...
- History source: `data/top15_tracker/archive/local_reset_20260413T151029/history.jsonl`
...
- Signal: `weak_1h_turn`
- Filters: `no_breakout_1h+chg_3_8`
- Stop / TP / max_h: recent front-high + 0.35x vol buffer | 1.0R | 12h
- Risk sizing: per-trade risk=0.25% equity | stop window=0.8%~4.5%
- Stats: n=10 / raw=17 | win_rate=0.8 | mean_equity_pnl=0.0824% | mean_R=0.3297 | pf=4.1408
- Stop profile: mean_stop=2.6133% | median_stop=2.669% | median_notional=9.4196%
- Exit mix: tp_rate=0.4 | sl_rate=0.1 | timeout_rate=0.5 | median_hours=8.7618
- Vol source mix: `{"kline_1h_atr14": 10}`

### `selective_structure_r2_h12`
- Signal: `weak_1h_turn`
- Filters: `no_breakout_1h+chg_3_8`
- Stop / TP / max_h: recent front-high + 0.35x vol buffer | 2.0R | 12h
- Risk sizing: per-trade risk=0.25% equity | stop window=0.8%~4.5%
- Stats: n=10 / raw=17 | win_rate=0.7 | mean_equity_pnl=0.0606% | mean_R=0.2424 | pf=2.1824
- Stop profile: mean_stop=2.6133% | median_stop=2.669% | median_notional=9.4196%
- Exit mix: tp_rate=0.2 | sl_rate=0.2 | timeout_rate=0.6 | median_hours=9.6062
- Vol source mix: `{"kline_1h_atr14": 10}`

## 7. Structure Stop Context
- Front-high lookback: 2h | volatility lookback: 1h | buffer multiplier: 0.35
- True ATR coverage: 32 / 32 events (1.0)
- Historical execution in this run mainly uses: `{"kline_1h_atr14": 32}`

## 8. Current Strategy Candidate
...
- Fixed reference: TP 2.0% / SL 1.0% / max 12h
  Stats: win_rate=0.8235 | mean_pnl=1.2619% | pf=8.1507
- Structure candidate: front-high + 0.35x vol buffer | TP 1.0R | risk 0.25% equity | max 12h
  Stats: win_rate=0.8 | mean_equity_pnl=0.0824% | mean_R=0.3297 | pf=4.1408
...
- The new structure stop uses true 1h ATR when available; otherwise it falls back to a 1h snapshot-range proxy.
- No funding, borrow cost, perp availability, or slippage filter is included in the historical execution layer yet.
